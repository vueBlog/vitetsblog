...
import { pageAsideAuthorModel, asideAuthorItemModel, articleItemModel } from './../types'
...
let aside = reactive<pageAsideAuthorModel>({
  loading: true,
  list: []
})
getAsideEvent(route, store).then(res => {
  // aside.loading = res?.loading as boolean
  // aside.list = res?.list as asideAuthorItemModel[]
  Object.assign(aside, res)
})

const activeName = computed(() => {
  return parseInt(route.query.author as string) || aside.list[0].authorId
})
let currentAuthor = reactive<asideAuthorItemModel>({
  authorEmail: '',
  authorHeadimg: '',
  authorId: 0,
  authorIntroduce: '',
...
function getCurrentAuthor(): void {
...
const getAuthorImg = (author: asideAuthorItemModel) => {
...
const list = reactive({
  ...
  justOriginal: false,
  order: 0,
  articleList: [] as articleItemModel[],
  total: 0,
  limit: 10,
  page: 1,
  dateTime: '',
